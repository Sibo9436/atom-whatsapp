...
export default class AtomWhatsappView {
  ...
  constructor(serializedState) {
    ...
    // Definition of function that will set colors to be used by insertCSS
    this.setColors = () => {
      const style = getComputedStyle(document.querySelector('.atom-whatsapp'));
      let atom_bg = style.getPropertyValue('--atom-app-background-color');
      let atom_bg_rgb = stupidRGB(atom_bg);
      let atom_bg_error = makeOpaque(style.getPropertyValue('--atom-background-color-error'), atom_bg);
      let atom_bg_highlight = makeOpaque(style.getPropertyValue('--atom-background-color-highlight'), atom_bg);
      let atom_bg_highlight_rgb = stupidRGB(atom_bg_highlight);
      let atom_bg_info = makeOpaque(style.getPropertyValue('--atom-background-color-info'), atom_bg);
      let atom_bg_selected = makeOpaque(style.getPropertyValue('--atom-background-color-selected'), atom_bg);
      let atom_bg_selected_rgb = stupidRGB(atom_bg_selected);
      let atom_bg_success = makeOpaque(style.getPropertyValue('--atom-background-color-success'), atom_bg);
      let atom_bg_warning = makeOpaque(style.getPropertyValue('--atom-background-color-warning'), atom_bg);
      let atom_fg = makeOpaque(style.getPropertyValue('--atom-text-color'), atom_bg);
      let atom_fg_error = makeOpaque(style.getPropertyValue('--atom-text-color-error'), atom_fg);
      let atom_fg_highlight = makeOpaque(style.getPropertyValue('--atom-text-color-highlight'), atom_fg);
      let atom_fg_highlight_rgb = stupidRGB(atom_fg_highlight);
      let atom_fg_info = makeOpaque(style.getPropertyValue('--atom-text-color-info'), atom_fg);
      let atom_fg_selected = makeOpaque(style.getPropertyValue('--atom-text-color-selected'), atom_fg);
      let atom_fg_subtle = makeOpaque(style.getPropertyValue('--atom-text-color-subtle'), atom_fg);
      let atom_fg_success = makeOpaque(style.getPropertyValue('--atom-text-color-success'), atom_fg);
      ...
      return `body{
        ...
        --incoming-background-deeper-rgb:${atom_bg_rgb} !important;
        ...
        --outgoing-background-deeper-rgb:${atom_bg_highlight_rgb} !important;
        --panel-background-colored:${atom_bg} !important;
        --panel-background-colored-deeper:${atom_bg} !important;
        --panel-background-deep:${atom_bg} !important;
    ...
    this.element.addEventListener('dom-ready',(e)=>{
      ...
      if (atom.config.get('atom-whatsapp.devToolsOnStart')) this.element.openDevTools();

      if (atom.config.get('atom-whatsapp.useAtomFont')) this.element.injectedCSS = this.element.insertCSS(`body{
        font-family: ${atom.config.get('editor').fontFamily} !important;`);

      if(atom.config.get('atom-whatsapp.useAtomThemes')) this.element.injectedCSS = this.element.insertCSS(this.setColors());
      atom.config.onDidChange('core.themes', () => {
        if (typeof(this.element.injectedCSS) !== 'undefined') {
          this.element.removeInsertedCSS(this.element.injectedCSS);
          console.log("Removed previous css");
        }
        if(atom.config.get('atom-whatsapp.useAtomThemes')) this.element.injectedCSS = this.element.insertCSS(this.setColors());
        console.log("Theme should now have changed");
      });
    });

    // atom.config.onDidChange('core.themes', () => this.element.src = this.element.src);
