...
export default class AtomWhatsappView {
  ...
  constructor(serializedState) {
    ...
    this.element.addEventListener('ipc-message',(message)=>{
      ...
      const sendCallback  = (text)=>console.log("Ciao!",text)
      this.pain = document.createElement('div');
      const notificationHTML =`<div>${notificationData.title}</div>
      <div class="text-smaller">${notificationData.body}</div>
      <div class="block">
      <img src="${notificationData.icon}">
      <input id="whatsapp-message" class="message input-text" type='text' placeholder='Answer me'/>
      </div>
      `
      atom.notifications.addInfo(notificationHTML, {buttons:[{className:"btn btn-info",text:"Reply",onDidClick:()=>sendCallback(document.getElementById('whatsapp-message').value)}]});
    });
