...
export default class AtomWhatsappView {
	...
	constructor(seriFontFamily, seriColors) {
		...
		this.element.addEventListener('dom-ready', async (e) => {
			await this.element.loadURL(e.url, {
				'userAgent': "Mozilla/5.0 (X11; Linux x86_64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/87.0.4280.88 Safari/537.36"
			});
		});

		// Create message element
		this.element.preload= path.join(__dirname, 'preload.js');

		this.element.addEventListener('ipc-message',(message)=>{
			const messageData = JSON.parse(message.channel);
			if (messageData.action === "reload") {
				this.element.reloadIgnoringCache();
			} else {
				...
				const notificationHTML = `<div>${messageData.title}</div>
					<div class="block">
					<div class="block">
					<img class="inline-block-tight" src="${messageData.icon}" style="border-radius:50%;width:50px;" >
					<div class="inline-block-tight text-smaller">${messageData.body}</div>
					</div>
					...
					buttons:[
						{
							...
							onDidClick:()=>{
								if (document.getElementById('whatsapp-message')) {
									replyCallback({chat:messageData,reply:document.getElementById('whatsapp-message').value});
								}else{
									sendCallback({chat:messageData});
								}
								notifica.dismiss();
							}
						}
